public class Board{
    ...
    boolean gameOver;
    byte[] compressed;
    HashMap<Integer,GamePiece> board;
    ...
    //takes in a compressed byte array
        ...
        for(int i=0; i<32; i++)
        {
            ...
            if(i%16 < 8) // Piece is a pawn
            {
                if((compressed[(color?32:33)] & (1 << (i%8)) >> (i%8)) == 1) // Pawn was promoted
                {
                    ...
                    try {board.put(pos, (GamePiece)promotedTypes[promotedIndex].getConstructor(params).newInstance(pos, hasMoved, color));}
                    catch(Exception e) {System.err.print(e);}
                }
                else
                    board.put(pos, new Pawn(pos, hasMoved, color));
                if(i == pawnIndex && enPassant)
                    enPawnsant = (Pawn)board.get(pos);
            }
            else
                try {board.put(pos, (GamePiece)types[i%16].getConstructor(params).newInstance(pos, hasMoved, color));}
                catch(Exception e) {System.err.print(e);}
        }
    }
